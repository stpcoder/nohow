...
async function expectCaptureIdle(page) {
  assert.equal(await page.locator('.nh-capture-target').count(), 0)
  assert.equal(await page.getByText('평소처럼 직접 업무를 수행해 주세요.', { exact: true }).count(), 1)
}

async function expectCaptureSuggestion(page, instruction) {
...
async function completeCaptureWork(page, paced = false) {
  ...
  await expectCaptureIdle(page)
  if (!paced) {
    await pause(page, 4700)
    await expectCaptureSuggestion(page, '보완 요청 메일을 클릭해 보실래요?')
  }
  await click(page.getByRole('button', { name: /TR-2026-0812 출장비 정산/ }), paced ? 2500 : 0)
  await expectCaptureIdle(page)
  if (!paced) {
    await pause(page, 4700)
    await expectCaptureSuggestion(page, '아래 작업표시줄에서 Excel을 열어 보실래요?')
  }
...
async function verifyDemo() {
  ...
  assert.equal(await page.getByRole('button', { name: /출장비 정산 보완/ }).count(), 1)
  assert.equal(await page.getByText('추천 매뉴얼', { exact: true }).count(), 1)
  await page.getByRole('textbox', { name: '업무 매뉴얼 검색' }).fill('정산')
  assert.equal(await page.getByRole('button', { name: /출장비 정산 보완/ }).count(), 1)
  assert.equal(await page.getByText('검색 결과 1건', { exact: true }).count(), 1)
