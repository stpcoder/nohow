...
function ManualLibrary({ query, onQuery, onOpen }: { query: string; onQuery: (value: string) => void; onOpen: () => void }) {
  ...
  return <section className="nh-library-modal"><header><span className="nh-brand"><i /> NoHow</span><div><CircleUserRound size={18} /> 이노하 · 재무지원팀</div></header><div className="nh-library-content"><p className="nh-eyebrow">다른 구성원의 업무 화면</p><h2>필요한 업무 매뉴얼을 찾아보세요.</h2><label className="nh-library-search"><Search size={20} /><input autoFocus aria-label="업무 매뉴얼 검색" value={query} onChange={(event) => onQuery(event.target.value)} placeholder="어떤 업무를 수행하려고 하나요?" /></label><div className="nh-library-result-heading"><p>{hasQuery ? '검색 결과 1건' : '추천 매뉴얼'}</p><span>{hasQuery ? '업무 목적과 사용 프로그램을 기준으로 찾았습니다.' : '검색하지 않아도 추천 매뉴얼을 바로 체험할 수 있습니다.'}</span></div><button className="nh-search-result nh-library-target" onClick={onOpen}><div className="nh-search-icon"><FileText size={23} /></div><div><span>{hasQuery ? '가장 관련 있는 매뉴얼' : '바로 체험할 수 있는 매뉴얼'} · 재무지원팀</span><h3>출장비 정산 보완</h3><p>메일 → Excel → 파일 탐색기 → 통합업무포털 · 5단계 · 약 4분</p></div><div className="nh-result-action">체험하기 <ArrowRight size={17} /></div></button><div className="nh-library-recent"><p>최근 많이 사용한 매뉴얼</p><span>법인카드 정산</span><span>휴가 신청 변경</span><span>출장 결과 보고</span></div></div></section>
}
